  ...
  it("renders the Table Cell (default)", () => {
    render(
      <table>
        <tbody>
          <tr>
            <StyledCell disabled={false} bgcolor="white">
              Table Default
            </StyledCell>
          </tr>
        </tbody>
    ...
    expect(tableElement).toHaveStyleRule("background-color", "white", {
      modifier: ":hover",
    });
  });
});
...
describe("Table Cell", () => {
  it("renders the Table Cell (disabled)", () => {
    ...
    expect(tableElement).toBeInTheDocument();
    expect(tableElement).toHaveStyle({ border: "2px solid black" });
    expect(tableElement).not.toHaveStyleRule("background-color", undefined, {
      modifier: ":hover",
    });
  });
});
